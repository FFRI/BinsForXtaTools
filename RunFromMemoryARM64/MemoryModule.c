...
FinalizeSection(PMEMORYMODULE module, PSECTIONFINALIZEDATA sectionData) {
	...
	const int ProtectionFlags[2][2][2] = {
		{
			// not executable
	...
	if (module->virtualProtect(sectionData->address, sectionData->size, protect, &oldProtect) == 0) {
...
__declspec(code_seg(".scode"))
static BOOL
OverwriteSecurityCookie(PMEMORYMODULE module, LPVOID originalImageBase)
{
	unsigned char* codeBase = module->codeBase;
	PIMAGE_LOAD_CONFIG_DIRECTORY loadConfig;

	PIMAGE_DATA_DIRECTORY directory = GET_HEADER_DICTIONARY(module, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG);
	if (directory->VirtualAddress == 0) {
		return TRUE;
	}

	loadConfig = (PIMAGE_LOAD_CONFIG_DIRECTORY)(codeBase + directory->VirtualAddress);

	unsigned char* pSecurityCookie = (loadConfig->SecurityCookie - (ULONGLONG)originalImageBase) + codeBase;
	*pSecurityCookie = 0;
...
BuildImportTable(PMEMORYMODULE module)
{
	...
	for (; !module->isBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
		uintptr_t *thunkRef;
		FARPROC *funcRef;
		HCUSTOMMODULE *tmp;
		...
		tmp = (HCUSTOMMODULE *) module->alloc(NULL,
			(module->numModules+1)*(sizeof(HCUSTOMMODULE)),
...
void RunFromMemoryARM64(char* buffer, size_t size) {
	PMEMORYMODULE result = (PMEMORYMODULE)MemoryLoadLibrary(buffer, size);
	result->exeEntry();
}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data, size_t size)
{
	...
	KERNELBASE_APIS apis = GetKernelBaseApis(&ntDllFuncs, TRUE);
	PMEMORYMODULE ret = MemoryLoadLibraryEx(data, size, &apis);
	ntDllFuncs.pNtFlushInstructionCache((HANDLE)-1, 0, 0);
	return ret;
}
...
HMEMORYMODULE MemoryLoadLibraryEx(const void *data,
	...
	apis->getNativeSystemInfo(&sysInfo);
	...
	LPVOID originalImageBase = (LPVOID)old_header->OptionalHeader.ImageBase;

	// reserve memory for image of library
	// XXX: is it correct to commit the complete memory region at once?
	//      calling DllEntry raises an exception if we don't...
	code = (unsigned char *)apis->allocMemory((LPVOID)(old_header->OptionalHeader.ImageBase),
		alignedImageSize,
		MEM_RESERVE | MEM_COMMIT,
		PAGE_READWRITE);
	...
	if (code == NULL) {
		...
		code = (unsigned char *)apis->allocMemory(NULL,
			alignedImageSize,
			MEM_RESERVE | MEM_COMMIT,
			PAGE_READWRITE);
		if (code == NULL) {
	...
	while ((((uintptr_t) code) >> 32) < (((uintptr_t) (code + alignedImageSize)) >> 32)) {
		POINTER_LIST *node = (POINTER_LIST*) apis->allocMemory(NULL,
			sizeof(POINTER_LIST),
			MEM_RESERVE | MEM_COMMIT,
			PAGE_READWRITE);
		if (!node) {
			apis->freeMemory(code, 0, MEM_RELEASE);
			FreePointerList(blockedMemory, apis->freeMemory);
			SetLastError(ERROR_OUTOFMEMORY);
			return NULL;
		}
	...
	result = (PMEMORYMODULE)apis->allocMemory(NULL,
		sizeof(MEMORYMODULE),
		MEM_RESERVE | MEM_COMMIT,
		PAGE_READWRITE);
	if (result == NULL) {
	...
	result->alloc = apis->allocMemory;
	result->free = apis->freeMemory;
	result->loadLibrary = apis->loadLibrary;
	result->getProcAddress = apis->getProcAddress;
	result->freeLibrary = apis->freeLibrary;
	result->virtualProtect = apis->virtualProtect;
	result->isBadReadPtr = apis->isBadReadPtr;
	...
	headers = (unsigned char*)apis->allocMemory(code,
	...
	// Overwrite security cookie
	if (!OverwriteSecurityCookie(result, originalImageBase)) {
		goto error;
	}
...
#ifdef _WIN64
	FreePointerList(module->blockedMemory, module->free);
#endif
